...
struct Solution
{
    ...
    long find_solution(const MatrixXc &puzzle, const Index &start, const Direction dirn)
    {
        MatrixXc energized(puzzle.rows(), puzzle.cols());
        energized.fill('.');

        project_ray(start, dirn, puzzle, energized);
        // std::cout << energized << "\n";
        return (energized.array() == '#').count();
...
int main()
{
    ...
    // First col, heading RIGHT
    long max_count = 0;
    for (int row = 0; row < puzzle.rows(); ++row)
    {
        Index start = {row, 0};
        Solution solution;
        long count = solution.find_solution(puzzle, start, RIGHT);
    ...
    for (int row = 0; row < puzzle.rows(); ++row)
    {
        Index start = {row, puzzle.cols() - 1};
        Solution solution;
        long count = solution.find_solution(puzzle, start, LEFT);
        max_count = std::max(max_count, count);
    }
    ...
    for (int col = 0; col < puzzle.cols(); ++col)
    {
        Index start = {0, col};
        Solution solution;
        long count = solution.find_solution(puzzle, start, DOWN);
        max_count = std::max(max_count, count);
    }
    ...
    for (int col = 0; col < puzzle.cols(); ++col)
    {
        Index start = {puzzle.rows() - 1, col};
        Solution solution;
        long count = solution.find_solution(puzzle, start, UP);
        max_count = std::max(max_count, count);
    }
